...
static const gchar		*currentLayoutN;
...
static void remove_button(GtkWindow *window) {
	...
	free((void *)wifi_state);
	free((void *)currentLayoutN);
	free((void *)wifi_connection_name);
	free((void *)wifi_connection_check);
...
static void on_volume_scroll(GtkEventController *volumeEvent) {
	const gchar *cmd = "pactl list sinks | \
						grep 'Volume:' | \
						head -n 1 | \
						awk '{print $5}' | \
						sed 's/%//'";

	const gchar *outputVol = output_to_char(cmd);
	...
	// if delta_y < 0 then touchpad scrolles up
    double delta_x;
    double delta_y;
    gdk_scroll_event_get_deltas(eventController, &delta_x, &delta_y);
    double delta_state;
    if (delta_y < 0) {
    	delta_state = 0;
    }
